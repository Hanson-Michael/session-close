...
final class AudioPreviewPlayer: ObservableObject {
    ...
    private func tick() {
        ...
        let time = sampleRate > 0 ? min(duration, Double(frame) / sampleRate) : 0
        // Deferred a turn rather than publishing directly from the timer
        // callback. The timer runs in .common RunLoop mode (deliberately —
        // see startPositionTimer, it needs to keep ticking during modal/
        // sheet transitions and tracking loops), which means it can fire
        // while SwiftUI is mid-way through its own view-update pass (e.g.
        // right as a sheet like the Spectrum window is being presented).
        // Publishing @Published `currentTime` synchronously in that window
        // is what SwiftUI's "Publishing changes from within view updates is
        // not allowed" warning is about. Wrapping in async hands the
        // mutation to the next run loop turn instead, after the in-flight
        // update has committed — at a 100ms tick rate this is imperceptible.
        DispatchQueue.main.async { [weak self] in
