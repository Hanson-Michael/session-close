...
enum SpectrumAnalyzer {
    ...
    static func analyze(file: AVAudioFile) throws -> [Double] {
        ...
        while start + fftSize <= monoSamples.count {
            ...
            let magnitudes = framePower(windowed: windowed, fftSetup: fftSetup, log2n: log2n)
        ...
        return bandCenterFrequenciesHz.map { center in
            ...
            // Divide by the full-scale calibration reference before taking
            // the log — this is what turns an arbitrary FFT-size-dependent
            // number into true dBFS (0 dB = as loud as a full-scale sine).
            // See `fullScaleReferencePower` below.
            return bandPower > 0 ? 10 * log10(bandPower / fullScaleReferencePower) : -160.0
        }
    }

    /// Runs one windowed real-FFT frame and returns per-bin power
    /// (magnitude-squared). Factored out so both the real measurement loop
    /// above and the one-time full-scale calibration below run through the
    /// exact same Accelerate packing/windowing math — if one ever changes,
    /// the other can't silently drift out of calibration with it.
    private static func framePower(windowed: [Float], fftSetup: FFTSetup, log2n: vDSP_Length) -> [Float] {
    ...
    /// Which FFT bin the calibration tone below is placed at — an
    /// arbitrary bin-aligned frequency (an exact integer number of cycles
    /// across the window), comfortably away from DC and Nyquist edge
    /// effects. This only determines a position in sample-index space, so
    /// the calibration is independent of any real file's sample rate — it's
    /// purely a property of fftSize and the window.
    private static let calibrationToneBin = fftSize / 8

    /// 0 dBFS reference: the single-frame FFT-bin power a full-scale
    /// (amplitude 1.0) bin-aligned sine tone produces through this exact
    /// same windowed real-FFT pipeline (`framePower`, same window, same
    /// packing). This is the standard way to calibrate a spectrum analyzer
    /// — "0 dB reads as loud as a full-scale sine" — and deriving it
    /// empirically like this, rather than hand-deriving Accelerate's
    /// real-FFT (zrip) packing and Hann window normalization constants
    /// analytically, guarantees it's self-consistent with the exact code
    /// path every real measurement runs through.
    ///
    /// Every band in `analyze()` is divided by this before converting to
    /// dB, so 0 dB now means true digital full scale rather than an
    /// arbitrary offset that shifted with fftSize/windowing — which is what
    /// previously made the spectrum overlay's axis show implausible values
    /// like +70 that no real (never-clipping) audio file could produce.
    private static let fullScaleReferencePower: Double = {
        var window = [Float](repeating: 0, count: fftSize)
        vDSP_hann_window(&window, vDSP_Length(fftSize), Int32(vDSP_HANN_NORM))

        var tone = [Float](repeating: 0, count: fftSize)
        let cyclesAcrossWindow = Double(calibrationToneBin)
        for n in 0..<fftSize {
            tone[n] = Float(sin(2.0 * Double.pi * cyclesAcrossWindow * Double(n) / Double(fftSize)))
        }

        var windowed = [Float](repeating: 0, count: fftSize)
        vDSP_vmul(tone, 1, window, 1, &windowed, 1, vDSP_Length(fftSize))

        let log2n = vDSP_Length(log2(Double(fftSize)))
        guard let fftSetup = vDSP_create_fftsetup(log2n, FFTRadix(kFFTRadix2)) else { return 1.0 }
        defer { vDSP_destroy_fftsetup(fftSetup) }

        let magnitudes = framePower(windowed: windowed, fftSetup: fftSetup, log2n: log2n)
        let peakPower = Double(magnitudes[calibrationToneBin])
        return peakPower > 0 ? peakPower : 1.0
    }()
